...
p.get = function(sid, cb)
{
    ...
    var table = azure.createTableService(this.config.name, this.config.accessKey, this.config.host, this.config.authenticationProvider);
    
    var me = this;
    table.queryEntity('AzureSessionStore', sid, '1', function(err, result) {
        if (err) {
            //console.log("AzureSessionStore.get: " + err);
            if (err.code == "ResourceNotFound") {
                cb();
            } else if (err.code == "TableNotFound") {
                table.createTableIfNotExists('AzureSessionStore', function(err){
                    if (err) {
...
p.set = function(sid, session, cb) {
    ...
    var me = this;
    table.insertOrReplaceEntity('AzureSessionStore', new_session, function(err, results) {
        if (err) {
...
p.destroy = function(sid, cb) {
    ...
    table.deleteEntity('AzureSessionStore', { PartitionKey : sid, RowKey : '1' } , function(err){
        if(err){
